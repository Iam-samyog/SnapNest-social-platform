...
// Sub-component to isolate the useGoogleLogin hook
const GoogleLoginButton = ({ onSuccess, onError, setLoading, setLoadingMessage, disabled }) => {
...
const SocialLogin = ({ onSuccess, onError }) => {
    ...
    return (
        <div className="space-y-3">
             {/* Render Google Button only if Client ID is present */}
            {GOOGLE_CLIENT_ID ? (
                <GoogleLoginButton 
                    onSuccess={onSuccess} 
                    onError={onError} 
                ...
                <button
                    type="button"
                    onClick={() => onError && onError('Google Client ID is not configured.')}
                    className="w-full bg-white border-2 border-black text-black py-3 rounded-lg font-semibold opacity-50 cursor-not-allowed flex items-center justify-center gap-2"
                >
